...
});item.querySelector('.whatsApp-btn').addEventListener('click', (e) => {
this.whatsApp(parseInt(item.dataset.id), e);
});
});
}
render() {
...
window.location.href = 'DetailsCarOnly2.html';
}
...
window.location.href = 'UpdateCar2.html';
this.render();
}
}
whatsApp(carId,event) {
event.stopPropagation();
if (confirm('Are you sure to send this car in whatsapp?')) {
sessionStorage.setItem( 'selectedCarToSend', JSON.stringify(carId));
window.location.href = 'FormWhatsApp.html';
this.render();
}
}
}
